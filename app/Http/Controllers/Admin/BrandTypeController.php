...
class BrandTypeController extends Controller
{
     ...
     * @return \Illuminate\Contracts\View\Factory|\Illuminate\View\View
     */
    public function postBrandTypeCreate(Request $request)
    {
        BrandType::create($request->all());
        return redirect(action('BrandTypeController@brandTypeLIst'));
    }
    ...
    {
        BrandType::where('id',$id)->update(['brandtype'=>$request->brandtype]);
        return redirect(action('BrandTypeController@brandTypeLIst'));
    }
